...
description: 了解如何使用 ViaProxy 设置 Geyser。
---
...
4. 打开您的 Geyser 配置文件，位于 `/plugins/Geyser/config.yml`，找到以下内容：
    ...
    如果您启用 `clone-remote-port`，端口将被覆盖并使用 ViaProxy 端口。
    由于您是自托管，您可以自由选择端口 - 默认端口是 19132。

    **重要**：其他依赖 UDP 端口的服务/插件，例如语音聊天或查询，不能与 Geyser 共享端口。

    如果您使用 ViaProxy GUI，您可以使用其账户管理系统让基岩版加入在线模式服务器。如果您使用 ViaProxy CLI 并希望允许基岩版玩家加入您的在线模式服务器，您必须安装并[设置 Floodgate](/wiki/floodgate/setup)。 <br/>

    注意：`auth-type` 字段的值会被 ViaProxy 自动忽略和管理。
...
:::info
要允许基岩版玩家无需登录付费 Java 版账户即可加入您的服务器，您可以使用 [Floodgate](/wiki/floodgate/setup)。
:::
